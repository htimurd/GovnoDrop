...
Все файлы лежат на одном уровне (без подпапок) — удобно загружать прямо
...
## Загрузка на GitHub без папок

1. Создай новый репозиторий на GitHub (Add file → Create new repository).
2. Add file → Upload files → перетащи все файлы из этого архива разом
   (они все на одном уровне, вложенных папок нет).
3. Commit changes.
...
Вебхуку нужен публичный HTTPS-адрес, поэтому для локального теста без
Render проще временно погонять бота через polling. Создай `poll.py`:
...
1. Залей все файлы в свой репозиторий на GitHub (см. раздел выше).
...
внешний managed Postgres/Render PostgreSQL и адаптируй `db.py`.

## Изменить баланс/шансы

Все тиры предметов, их эмодзи, стоимость, вес дропа и шанс апгрейда
задаются в одном месте — `items.py`, список `TIERS`. Правь его, чтобы
настроить экономику игры.
